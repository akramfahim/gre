<?php 
        ...
        if (isset($_POST['level_four_submit'])) {
           if(empty($_POST['answer0']) || empty($_POST['answer1']) || empty($_POST['answer2']) ) {
                ...
                $level_four_ques_id = $_POST['question_id0'];
                $level_four_ques_id_two = $_POST['question_id1'];
                $level_four_ques_id_three = $_POST['question_id2'];
                ...
                if (
                    ...
                    $level_four_sql1 = $pdo->prepare("SELECT * FROM level_settings WHERE user_id=?");
                    $level_four_sql1->execute(array($_POST['user_id']));
                    $level_four_totalrow = $level_four_sql1->rowCount();              
                    if($level_four_totalrow) {

                        $level_four_sql = $pdo->prepare("UPDATE level_settings SET level_Four=? WHERE user_id= ?");
