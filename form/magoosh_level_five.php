           ...
           if(empty($_POST['answer0']) || empty($_POST['answer1']) || empty($_POST['answer2']) ) {
                ...
                if (
                    ...
                    if($level_five_totalrow) {
                        ...
                        $level_five_sql = $pdo->prepare("UPDATE level_settings SET level_Five=? WHERE user_id= ?  AND type='magoosh'");
                        $level_five_sql->execute(array('Completed',$_POST['user_id']));

                          $setting = $pdo->prepare("UPDATE settings SET magoosh=? WHERE user_id= ?");
